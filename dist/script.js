...
(function (p5) {
    ...
    class AsteroidController {
        constructor(p, scale = 1) {
            ...
            this.getRandomVector = (scalar) => {
                return this.p.createVector((Math.random() * scalar * 2) - (scalar), (Math.random() * scalar * 2) - (scalar));
            };
            ...
            this.spawnAsteroidWave = (levelNum) => {
                for (let i = 0; i < 4; i++) {
                    this.spawnLargeAsteroid(this.getRandomAsteroidSpawnPosition(), this.getRandomVector(1));
                }
            };
            ...
            this.spawnLargeAsteroid = (position, velocity) => {
                ...
                this.asteroids.push(new LargeAsteroid(this.p, newPos, largeAsteroidTypes[randomIndex], this.scale, velocity));
            };
            this.spawnMediumAsteroid = (position, velocity) => {
                ...
                this.asteroids.push(new MediumAsteroid(this.p, newPos, mediumAsteroidTypes[Math.floor(Math.random() * mediumAsteroidTypes.length)], this.scale, velocity));
            };
            this.spawnSmallAsteroid = (position, velocity) => {
                ...
                this.asteroids.push(new SmallAsteroid(this.p, newPos, asteroidType, this.scale, velocity));
            };
                ...
                for (let i = 0; i < bullets.length; i++) {
                    for (let j = 0; j < this.asteroids.length; j++) {
                        if ((this.asteroids[j]).checkCollision(bullets[i])) {
                            ...
                            const asteroidType = (_a = this.asteroids[j]) === null || _a === void 0 ? void 0 : _a.getType();
    ...
    class Bullet {
        constructor(p, pos, vel, rot, creationTime) {
            this.scale = 1;
            this.lifespan = 180;
            this.p = p;
            this.pos = pos;
    ...
    class PlayerShip {
        ...
        addThrust() {
            const thrustVector = this.p.createVector(this.thrust * Math.cos(this.rot), this.thrust * Math.sin(this.rot));
        ...
        addBullet() {
            if (this.coolDown === 0) {
                this.bullets.push(new Bullet(this.p, this.p.createVector(this.pos.x, this.pos.y), this.p.createVector(Math.cos(this.rot) * 5, Math.sin(this.rot) * 5), this.rot, this.p.frameCount));
                this.coolDown = 16;
            }
        }
        getPos() {
        ...
        draw(numLives) {
            ...
            for (let i = 0; i < numLives; i++) {
                if (p.frameCount % 3 === 0) {
                    ...
                    PlayerDisplay.drawPlayer(p, p.createVector(115 + ((i * 16) * this.scale), (135) * this.scale), -p.HALF_PI, this.scale, '#0a0');
                }
            }
            p.pop();
        }
    ...
    class GameController {
        ...
        canReset() {
            ...
            for (let i = 0; i < theAsteroids.length; i += 1) {
                if ((((_a = theAsteroids[i]) === null || _a === void 0 ? void 0 : _a.getDistanceTo(spawnPoint)) !== null)
        ...
        advance() {
            ...
            if (playerCollision) {
                ...
                this.asteroidController.addExplosion(new Explosion(this.p, explosionPos));
                this.numLives -= 1;
        ...
        static resetInstance() {
            var _a, _b;
            const p = (_a = GameController.instance) === null || _a === void 0 ? void 0 : _a.p;
            const font = (_b = GameController.instance) === null || _b === void 0 ? void 0 : _b.font;
    ...
    SVGArc.getAdjustedCoordinates = (xRotTheta, x1, y1, x2, y2) => {
        ...
        const x1_prime = Math.cos(xRotTheta) * (x1 - x_m) - Math.sin(xRotTheta) * (y1 - y_m);
        const y1_prime = -Math.sin(xRotTheta) * (x1 - x_m) + Math.cos(xRotTheta) * (y1 - y_m);
    ...
    SVGArc.computeLambda = (xRotTheta, x1, y1, x2, y2, radiusX, radiusY) => {
        ...
        const a = radiusX;
        const b = radiusY;
        const { x1_prime, y1_prime } = SVGArc.getAdjustedCoordinates(xRotTheta, x1, y1, x2, y2);
    ...
    SVGArc.computeCenterOffset = (xRotTheta, x1, y1, x2, y2, radiusX, radiusY) => {
        const lambda = SVGArc.computeLambda(xRotTheta, x1, y1, x2, y2, radiusX, radiusY);
        const { x1_prime, y1_prime } = SVGArc.getAdjustedCoordinates(xRotTheta, x1, y1, x2, y2);
        const a = radiusX;
        const b = radiusY;
    ...
    SVGArc.transformBack = (xRotTheta, x1, y1, x2, y2, radiusX, radiusY) => {
        const { x_m, y_m } = SVGArc.getMidpoint(x1, y1, x2, y2);
        const { cx_prime, cy_prime } = SVGArc.computeCenterOffset(xRotTheta, x1, y1, x2, y2, radiusX, radiusY);
        const h = x_m + (cx_prime * Math.cos(xRotTheta) - cy_prime * Math.sin(xRotTheta));
        const k = y_m + (cx_prime * Math.sin(xRotTheta) + cy_prime * Math.cos(xRotTheta));
        return { h, k };
    };
    ...
    SVGCircle.process = (element) => {
        const rString = element === null || element === void 0 ? void 0 : element.getAttribute("r");
        const cxString = element === null || element === void 0 ? void 0 : element.getAttribute("cx");
        const cyString = element === null || element === void 0 ? void 0 : element.getAttribute("cy");
        let radius;
        let cx;
        let cy;
    ...
    SVGEllipse.process = (element) => {
        ...
        const cx = element === null || element === void 0 ? void 0 : element.getAttribute("cx");
        const cy = element === null || element === void 0 ? void 0 : element.getAttribute("cy");
        const rx = element === null || element === void 0 ? void 0 : element.getAttribute("rx");
        const ry = element === null || element === void 0 ? void 0 : element.getAttribute("ry");
        let centerPt;
        let radiusX;
        let radiusY;
        if (typeof cx === "string" &&
    ...
    SVGEllipse.draw = (renderer, element) => {
        ...
        renderer.ellipse(ellipse.center.x, ellipse.center.y, ellipse.radiusX, ellipse.radiusY);
        renderer.pop();
    };
    ...
    SVGImage.process = (element, renderer) => {
        ...
        const xString = (element === null || element === void 0 ? void 0 : element.getAttribute("x")) || null;
        const yString = (element === null || element === void 0 ? void 0 : element.getAttribute("y")) || null;
        const widthString = (element === null || element === void 0 ? void 0 : element.getAttribute("width")) || null;
        const heightString = (element === null || element === void 0 ? void 0 : element.getAttribute("height")) || null;
        let x = 0;
        let y = 0;
        let w = null;
        let h = null;
    ...
    SVGImage.draw = (renderer, element) => {
        if (element.type === SVGObjectType$1.IMAGE) {
            ...
            if (svgImage.img) {
                renderer.push();
                renderer.image(svgImage.img, svgImage.position.x, svgImage.position.y, svgImage.width, svgImage.height);
                renderer.pop();
            }
            else {
    ...
    SVGLine.process = (element) => {
        // static processLine = (line: Element): SVGLine | null => {
        const x1String = element === null || element === void 0 ? void 0 : element.getAttribute("x1");
        const y1String = element === null || element === void 0 ? void 0 : element.getAttribute("y1");
        const x2String = element === null || element === void 0 ? void 0 : element.getAttribute("x2");
        const y2String = element === null || element === void 0 ? void 0 : element.getAttribute("y2");
        let x1;
        let y1;
        let x2;
        let y2;
    ...
    SVGPath.process = (element) => {
        var _b, _c, _d;
        const instructionsAsStringList = ((_b = element === null || element === void 0 ? void 0 : element.getAttribute("d")) === null || _b === void 0 ? void 0 : _b.split(" ")) || null;
        ...
        if (instructionsAsStringList) {
            ...
            for (let p = 0; p < instructionsAsStringList.length; p += 1) {
                ...
                if (param && param !== "") {
                    ...
                    if (param === "Z") {
                        const lastPt = (_c = lines[lines.length - 1]) === null || _c === void 0 ? void 0 : _c.p2;
                        const firstPt = (_d = lines[0]) === null || _d === void 0 ? void 0 : _d.p1;
                        if (lastPt && firstPt) {
                            const lineParams = {
                                style: {},
                                p1: lastPt,
    ...
    SVGRect.process = (element) => {
        const xString = (element === null || element === void 0 ? void 0 : element.getAttribute("x")) || "0";
        const yString = (element === null || element === void 0 ? void 0 : element.getAttribute("y")) || "0";
        const widthString = element === null || element === void 0 ? void 0 : element.getAttribute("width");
        const heightString = element === null || element === void 0 ? void 0 : element.getAttribute("height");
        let x;
        let y;
        let width;
        let height;
    ...
    SVGPolygon.process = (element) => {
        var _b;
        const pointsAsStringList = ((_b = element === null || element === void 0 ? void 0 : element.getAttribute("points")) === null || _b === void 0 ? void 0 : _b.split(" ")) || null;
        let ptArray;
        if (pointsAsStringList) {
    ...
    class SVGLoader {
        ...
        static processSVG(data) {
            var _b;
            const svgString = data === null || data === void 0 ? void 0 : data.join("\n");
            const parser = new DOMParser();
            const doc = parser.parseFromString(svgString, "image/svg+xml");
            // Get class from polygon, lines, circles
            // const svgElementClass =
            (_b = this.svgPolygonElementList[0]) === null || _b === void 0 ? void 0 : _b.getAttribute("class");
    ...
    SVGLoader.loadSVG = (filename = null) => {
        if (filename) {
            _a.p.loadStrings(filename, (data) => _a.handleSvgData(data) // callback
            );
        }
    };
    SVGLoader.processSVGStyle = (svgStyleData
    // svgStyleName: string,
    // svgStyleData: string
    ) => {
        var _b, _c, _d, _e;
        ...
        if (svgStyleData.length > 0) {
            ...
            let styleName = ((_c = (_b = svgStyleData[0]) === null || _b === void 0 ? void 0 : _b.innerHTML) === null || _c === void 0 ? void 0 : _c.split("{")[0]) || ""; // .st0
            styleName = styleName.replace(/[\t\n\r]/gm, "");
            const styleAttributesString = ((_e = (_d = svgStyleData[0]) === null || _d === void 0 ? void 0 : _d.innerHTML) === null || _e === void 0 ? void 0 : _e.split("{")[1]) || ""; // fill: #FFFFFF; stroke: #000000; stroke-miterlimit: 10;
    ...
    let font;
    let gameController;
    const preload = (p) => __awaiter(void 0, void 0, void 0, function* () {
