...
(function (p5) {
  ...
  class SVGArc extends SVGObject {
    ...
    constructor(params) {
      ...
      this.toSVGString = () => {
        var _a, _b;
        return `A ${this.radiusX} ${this.radiusY} ${this.xAxisRotation} 0 0 ${
          (_a = this.endPosition) === null || _a === void 0 ? void 0 : _a.x
        } ${(_b = this.endPosition) === null || _b === void 0 ? void 0 : _b.y}`;
  ...
  SVGCircle.process = (element) => {
    ...
    if (rString && cxString && cyString) {
      ...
      if (
        ...
        const circle = SVGFactory$1.createCircle({
  ...
  SVGEllipse.process = (element) => {
    ...
    if (
      ...
      if (
        ...
        const ellipse = SVGFactory$1.createEllipse(params);
        return ellipse;
      }
    }
    return null;
  ...
  SVGImage.process = (element, renderer) => {
    ...
    if (
      ...
      const image = SVGFactory$1.createImage(imgData);
      image.img = img;
      return image;
    }
    return null;
  ...
  SVGLine.process = (element) => {
    ...
    if (x1String && y1String && x2String && y2String) {
      ...
      if (
        ...
        const line = SVGFactory$1.createLine({
          p1: new Point(x1, y1),
          p2: new Point(x2, y2),
        });
        return line;
  ...
  SVGRect.process = (element) => {
    ...
    if (xString && yString && widthString && heightString) {
      ...
      if (
        ...
        const rect = SVGFactory$1.createRect(params);
        return rect;
      }
    }
    return null;
  ...
  SVGFactory.createPolygon = (params) => {
    return new SVGPolygon$1(params);
  };
  ...
  SVGPolygon.create = (points) => {
    return SVGFactory$1.createPolygon({ points });
  };
  ...
  SVGPolygon.process = (element) => {
    ...
    let ptArray;
    if (pointsAsStringList) {
      ptArray = SVGFactory$1.pointStringListToPointObjArray(pointsAsStringList);
      if (ptArray) {
        const polygon = SVGFactory$1.createPolygon({
          points: ptArray,
        });
        return polygon;
      }
  ...
  class SVGLoader {
    ...
    static processSVG(data) {
      ...
      this.polygons.push(...SVGPolygon$1.processList(doc));
  ...
  SVGLoader.drawAll = () => {
    SVGPolygon$1.drawList(SVGLoader.p, SVGLoader.polygons);
